...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [[AudioManager sharedInstance] playSongWithURI:((Track *)([givenPlaylist tracks][indexPath.row])).uri];
    [tableView deselectRowAtIndexPath:indexPath animated:YES];
}

- (void)tableView:(UITableView *)tableView didHoldRowAtIndexPath:(NSIndexPath *)indexPath {
    
    NSURL *musicURI = [[givenPlaylist.tracks objectAtIndex:indexPath.row] uri];
    ...
    for (TrackPlaylist *playlist in [[AudioManager sharedInstance] playlists]) {
        if ([playlist isEqual:givenPlaylist]) {
            continue;
        }
